...
import { fixtureApi, fixtureNotes, type FixtureNote } from "./fixtures";
...
function Tree({ activeId, onSelect, hoisted }: { activeId: string; onSelect: (id: string) => void; hoisted: boolean }) {
  const roots = fixtureNotes.filter((note) => (hoisted ? note.id === activeId : note.parents.length === 0));
  const childrenOf = (id: string) => fixtureNotes.filter((note) => note.parents.includes(id));

  const branch = (note: FixtureNote, depth: number) => (
...
function Sidebar({ activeId, onSelect, hoisted, onToggleHoist }: { activeId: string; onSelect: (id: string) => void; hoisted: boolean; onToggleHoist: () => void }) {
  return (
    <aside className="sidebar">
      ...
      <div className="tree-heading"><span>All notes</span><span className="count-pill">{fixtureNotes.length}</span></div>
      <Tree activeId={activeId} onSelect={onSelect} hoisted={hoisted} />
...
function Tabs({ tabs, activeId, onSelect, onClose }: { tabs: string[]; activeId: string; onSelect: (id: string) => void; onClose: (id: string) => void }) {
  return <div className="tabs" role="tablist">{tabs.map((id) => { const note = fixtureNotes.find((item) => item.id === id)!; return <div key={id} className={`tab ${activeId === id ? "is-active" : ""}`} role="tab" aria-selected={activeId === id}><button onClick={() => onSelect(id)}><span className="tab-icon">{note.icon}</span>{note.title}</button><button className="tab-close" onClick={() => onClose(id)} aria-label={`Close ${note.title}`}>×</button></div>; })}<button className="new-tab" aria-label="New tab">＋</button></div>;
}

function NotePane({ note, split, onSplit }: { note: FixtureNote; split: boolean; onSplit: () => void }) {
...
function ContextPanel({ note, open, onToggle }: { note: FixtureNote; open: boolean; onToggle: () => void }) {
...
export function App() {
  ...
  const activeId = routeId && fixtureNotes.some((note) => note.id === routeId) ? routeId : state.activeId;
  const activeNote = useQuery({ queryKey: ["fixture-note", activeId], queryFn: () => fixtureApi.note(activeId) }).data ?? fixtureNotes[0];
  const workspace = useQuery({ queryKey: ["fixture-workspace"], queryFn: fixtureApi.workspace });
  const results = useQuery({ queryKey: ["fixture-search", query], queryFn: () => fixtureApi.search(query), enabled: searchOpen });
  ...
  return <div className="app-shell"><LaunchBar query={query} setQuery={setQuery} onSearch={openSearch} />{searchOpen && <div className="search-popover"><div className="search-popover-head"><span>Quick search</span><button onClick={() => setSearchOpen(false)}>Esc</button></div>{results.isLoading ? <div className="search-empty">Searching…</div> : results.data?.length ? results.data.map((result) => <button className="search-result" key={result.id} onClick={() => select(result.id)}><span className="search-result-icon">{result.icon}</span><span><strong>{result.title}</strong><small>{result.path}</small></span><kbd>↵</kbd></button>) : <div className="search-empty">No matching notes</div>}</div>}<div className="workspace-layout"><Sidebar activeId={activeId} onSelect={select} hoisted={state.hoisted} onToggleHoist={() => dispatch({ type: "toggle-hoist" })} /><main className="main-stage"><Tabs tabs={state.tabs} activeId={activeId} onSelect={select} onClose={(id) => dispatch({ type: "close", id })} /><div className="content-stage"><NotePane note={activeNote} split={state.split} onSplit={() => dispatch({ type: "toggle-split" })} />{state.split && <NotePane note={fixtureNotes.find((note) => note.id === (state.tabs.find((tab) => tab !== activeId) ?? "welcome"))!} split={false} onSplit={() => dispatch({ type: "toggle-split" })} />}<ContextPanel note={activeNote} open={state.contextOpen} onToggle={() => dispatch({ type: "toggle-context" })} /></div><footer className="status-bar"><span><span className="online-dot" /> local vault</span><span>{status}</span><span>readonly fixture</span><span className="status-spacer" /><span>focus: {state.focus}</span><span>⌘ P commands</span></footer></main></div></div>;
}
