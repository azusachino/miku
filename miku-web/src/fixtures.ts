...
const wait = <T,>(value: () => T): Promise<T> =>
  new Promise((resolve) => window.setTimeout(() => resolve(value()), 80));

export const fixtureApi = {
  workspace: () =>
    wait(() => ({ noteCount: fixtureNotes.length, placementCount: fixtureTree.length, readonly: true })),
  tree: (parentId: string | null) =>
    wait(() => fixtureTree.filter((node) => node.parentId === parentId)),
  note: (id: string) => wait(() => fixtureNotes.find((note) => note.id === id) ?? fixtureNotes[0]),
  search: (query: string) =>
    wait(() => {
      const normalized = query.trim().toLowerCase();
      return fixtureNotes.filter(
        (note) => !normalized || `${note.title} ${note.path} ${note.body}`.toLowerCase().includes(normalized),
      );
    }),
};
