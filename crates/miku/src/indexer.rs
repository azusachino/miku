...
#[cfg(test)]
const BENCHMARK_VAULT_ENV: &str = "MIKU_BENCHMARK_VAULT";
...
async fn reconcile_store(
    ...
    for file in files {
        ...
        {
            unchanged_pages += 1;
            unchanged_files.push(PendingSource {
...
mod tests {
    ...
    async fn benchmark_real_vault_reconcile() {
        use miku_domain::{IndexReader, IndexWriter};
        ...
        let mut files = Vec::new();
        walk_store_tree(&content_root, &mut files).expect("walk benchmark vault");
        let bytes = files
            .iter()
            .map(fs::metadata)
            .collect::<std::io::Result<Vec<_>>>()
            .expect("stat benchmark vault")
            .into_iter()
