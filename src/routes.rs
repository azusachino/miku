...
#[derive(serde::Deserialize)]
pub(super) struct TrashForm {
    path: String,
}

#[derive(serde::Deserialize)]
pub(super) struct TrashIdForm {
    id: String,
}
...
pub(super) fn safe_trash_id(id: &str) -> Result<(), AppError> {
...
pub(super) async fn page_trash(Json(form): Json<TrashForm>) -> Result<Response, AppError> {
    ...
    if let Err(error) = fs::write(dir.join(format!("{id}.json")), manifest_json) {
        let _ = fs::rename(&trash_md, &src);
...
pub(super) async fn trash_list() -> Result<Response, AppError> {
    let dir = trash_dir();
...
pub(super) async fn trash_purge(Json(form): Json<TrashIdForm>) -> Result<Response, AppError> {
    safe_trash_id(&form.id)?;
    let dir = trash_dir();
    for ext in ["md", "json"] {
