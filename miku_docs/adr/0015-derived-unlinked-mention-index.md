...
The background indexer will maintain a relation from a source page to a target page when the source contains the target's title or alias as plain Markdown text, excluding existing wikilinks, code,
frontmatter, and self-references. The page route will only read this relation; it will never scan the vault, invoke body FTS, or reread candidate Markdown files to discover mentions.
...
- The relation is eventually consistent. A newly changed page may not appear in unlinked mentions until its background batch commits; this is acceptable because unlinked mentions are a secondary
  discovery surface.
