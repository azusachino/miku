...
Proposed and accepted; not yet implemented. `crates/miku-index-memory` still stores pages in a single `BTreeMap<String, PageIndex>` with a full-corpus `rebuild_backlinks()` on every write, and `crates/miku-index-sqlite` still performs the relational `tb_links` sweep this ADR replaces. Adoption is tracked as `miku:document-graph-index` in asobi.
